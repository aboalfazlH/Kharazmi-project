...
class SignUpForm(forms.Form):
    ...
    first_name = forms.CharField(max_length=50, widget=forms.TextInput(attrs={'placeholder': _('نام')}),required=False,label=_("نام"))
    last_name = forms.CharField(max_length=50, widget=forms.TextInput(attrs={'placeholder': _('نام خانوادگی')}),required=False,label=_("نام خانوادگی"))
    email = forms.EmailField(widget=forms.EmailInput(attrs={'placeholder': _('ایمیل')}),required=False,label=_("ایمیل"))
...
class LoginForm(forms.Form):
    ...
    def clean(self):
        ...
        if not email and not phone_number and not username:
            raise forms.ValidationError(_("برای ورود حداقل یا شماره تلفن،یا ایمیل یا نام کاربری خود را وارد کنید"))
        return cleaned_data
